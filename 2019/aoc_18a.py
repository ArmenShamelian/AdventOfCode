...
def iterate_bfs(map_, start, keys, prev_steps, path=[], fromkey='@'):
    """

    :param map_: Dict of (x,y) --> character at that position
    :param start: Position where the robot starts
    ...
    while queue:
        ...
        if position in keys.keys():
            keys_found[position] = map_[position]
    ...
    for pos in keys_found.keys():
        ...
        steps_this_route, this_path = iterate_bfs(map_, pos, new_keys, new_prev, path=path+[key], fromkey=key)
...
def run_a():
    input_ = load_input()
    map_, start, keys, doors = build_map(input_)
    min_steps, best_path = iterate_bfs(map_, start, keys, 0)
